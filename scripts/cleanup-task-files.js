...
const tempPatterns = [
  ...
  'spa-detection-new-sites-*.json',
  'spa-detection-updated.json',
  'temp-*.js',
  '*.txt',
  '*-method.txt',
  '*-method.js',
...
function cleanOldArchiveFiles(maxAgeMs = 24 * 60 * 60 * 1000) {
  const now = Date.now();
  const archives = [logsArchiveDir, rootArchiveDir];
  const deleted = [];
  
  for (const archiveDir of archives) {
    ...
    for (const file of files) {
      ...
      if (age > maxAgeMs) {
        ...
        deleted.push(`${path.basename(archiveDir)}/${file}`);
      }
    }
  }
...
function main() {
  ...
  // Clean old files in archive (older than 1 day)
  const deleted = cleanOldArchiveFiles();
  if (deleted.length > 0) {
    console.log(`\n🗑️  Deleted ${deleted.length} old files from archive:`);
  ...
  const logEntry = `\n## ${timestamp}\n- Task: Enhanced cleanup (v2)\n- Moved: ${totalMoved.length} files\n${totalMoved.map(f => `  - ${f}`).join('\n')}\n- Deleted from archive: ${deleted.length} files\n${deleted.map(f => `  - ${f}`).join('\n')}\n- Kept: ${totalKept.length} files\n`;
  fs.appendFileSync(cleanupLog, logEntry);
  
  console.log('\n' + '='.repeat(50));
  console.log(`✅ Cleanup completed: moved ${totalMoved.length} files to archive, deleted ${deleted.length} old files, kept ${totalKept.length} files.`);
}
